...
int seleccionInvalida(int motor_index, int ala_index) {
    if (motor_index == 3 && ala_index == 4) {
        return 1;
    }
    if (motor_index == 1 && ala_index == 4) {
        return 1;
    }
    if (motor_index == 2 && (ala_index == 2 || ala_index == 3)) {
        return 1;
    }
    return 0;
}
...
int main() {
    ...
    do {
        ...
        categoria_index = seleccionarOpcion("Seleccione la categoria (Servicio = Avion aun en servicio,  Descontinuado = El avion ya ha sido descontinuado)", categorias, 3);
